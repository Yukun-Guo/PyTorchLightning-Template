...
class DataModel(plt.LightningDataModule):
    ...
    def __init__(self, file_list: list, data_split_idx: list, img_size: tuple = (384, 288), batch_size: int = 32, img_tag='imgMat', msk_tag='imgMask'):
        super().__init__()
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        self.train_dataset = myDataset_mat(
            train_list, self.img_size, img_tag=self.img_tag, msk_tag=self.msk_tag)
        self.valid_dataset = myDataset_mat(
            valid_list, self.img_size, img_tag=self.img_tag, msk_tag=self.msk_tag)
        print(
