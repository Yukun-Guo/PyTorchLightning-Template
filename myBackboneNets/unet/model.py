...
class Unet(nn.Module):
    def __init__(
        ...
        assert len(encoder_channels) == len(decoder_channels), "Encoder channel number must equal to decoder channel number!"
    ...
    def _make_encoder(self):
        ...
        for i in range(self.sample_depth):
            if i == 0:
                block1 = encoder_block(self.in_channels, self.encoder_channels[i],kernel_size=3)
                blocks = nn.Sequential(*[block1,*[encoder_block(self.encoder_channels[i],self.encoder_channels[i],3) for j in range(self.encoder_blockdepth[i]-1)]])
            else:
                pooling = pooling_layer(self.encoder_channels[i-1],self.encoder_channels[i],kernel_size=2,stride=2)
                blocks = nn.Sequential(*[pooling,*[encoder_block(self.encoder_channels[i],self.encoder_channels[i],3) for j in range(self.encoder_blockdepth[i])]])
    ...
    def _make_decoder(self):
        ...
        for i in range(self.sample_depth):
            if i == self.sample_depth-1:
                block1 = decoder_block(self.decoder_channels[i]+self.encoder_channels[-(i+1)],self.decoder_channels[i],3)
                blocks = nn.Sequential(*[block1,*[decoder_block(self.decoder_channels[i],self.decoder_channels[i],3) for j in range(self.decoder_blockdepth[i]-1)]])
            else:
                block1 = decoder_block(self.decoder_channels[i]+self.encoder_channels[-(i+1)],self.decoder_channels[i],3)
                upsample = upsample_layer(self.decoder_channels[i],self.decoder_channels[i+1],kernel_size=3)
                blocks = nn.Sequential(*[block1,*[decoder_block(self.decoder_channels[i],self.decoder_channels[i],3) for j in range(self.decoder_blockdepth[i]-1)],upsample])
    ...
    def _make_center_block(self):
        ...
        block_in = encoder_block(self.encoder_channels[self.sample_depth-1],self.center_channels,kernel_size=3)
        block_out = encoder_block(self.center_channels,self.decoder_channels[0],kernel_size=3)
        blocks = nn.Sequential(*[block_in,*[encoder_block(self.center_channels,self.center_channels,3) for j in range(self.center_blockdepth)],block_out])
        return blocks

    def _make_output(self):
        decoder_block = base.getBaseBlock(self.decoder_blocktype)
        output = decoder_block(self.decoder_channels[self.sample_depth-1],self.classes,3)
        return output
...
if __name__ == '__main__':
    ...
    class VisualizeModelStructure(ptl.LightningModule):
        ...
        def summary(self):
            self.to_onnx(file_path=self.model_name+'.onnx')
